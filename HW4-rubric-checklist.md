...
## A. Kubernetes deployment — graded core (55 pts)
...
| ☐ | **HPA** targets the Deployment, sets min/max, shows a real CPU `TARGETS` value (metrics-server working) | `hpa.yaml` + `kubectl get hpa` | 10 |
...
| ☐ | **Public `/predict` URL returns 200** | `smoke_test.sh` against the FQDN | 10 |
...
- Accept **local (Minikube/kind) full marks**; the required Azure step is its own 20 pts; do **not**
